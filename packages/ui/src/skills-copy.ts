...
const SKILLS_COPY = {
  ...
  en: {
    ...
    row: { hoverWithTools: (id, runtime, status, tools) => `Skill: ${id}\n\nRuntime status: ${runtime}\nSource status: ${status}\nDeclared tools: ${tools}\nPermissions still follow the current session policy; this is not authorization.`, hover: (id, runtime, status) => `Skill: ${id}\n\nRuntime status: ${runtime}\nSource status: ${status}`, opening: 'Opening…', updating: 'Updating…', toggling: 'Switching…', reviewing: 'Reviewing…', useAriaLabel: (name) => `Use ${name} in chat`, use: 'Use', openAriaLabel: (name) => `Open SKILL.md for ${name}`, openTitle: 'Open SKILL.md', disableAriaLabel: (name) => `Disable ${name}`, enableAriaLabel: (name) => `Enable ${name}`, stateErrorTitle: 'The Skill state file for this project is invalid', enabledTitle: 'Agents in this project can use this skill', disabledTitle: 'Agents in this project will not see or load this skill', viewDiff: 'View diff', viewUpdate: 'View update', confirmDeleteAriaLabel: (name) => `Confirm deletion of ${name}`, deleteAriaLabel: (name) => `Delete ${name}`, confirmDelete: 'Confirm delete', delete: 'Delete' },
